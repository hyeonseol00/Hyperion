...
void Enemy4::Init()
{
	OBJMAN()->LoadData(L"Data\\Enemy4.png", 200, 100, &pTex);
	vPos = { 0, 0, 0 };
	rRect = { 0, 0, 100, 100 };
	...
	for (int i = 0; i < 2; i++)
		m_eEnemy4Division[i] = new Enemy4Division;
}
...
void Enemy4::Down()
{
	if (bVisible && iHP <= 0)
	{
		bVisible = false;
		for (int i = 0; i < 2; i++)
			m_eEnemy4Division[i]->Spawn(vPos, rRect, i);
		bEffectVisible = true;
		iEffectTime = 0;
		iEffectCount = 0;
...
void Enemy4::Hit(int iDmg)
{
	iHP -= iDmg;
	OBJMAN()->PlusScore(100);
}

void Enemy4::OutScreen()
{
